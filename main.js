	...
	for(let i=0;i<arr.length;i++){
		...
		$('#myWH').append(
			...
			(moment.duration(moment().diff(moment(moment.unix(myWH[i][2])))).asSeconds()>/*60480*/0&&breedable==true?
				myWH[i][7]+'" onclick="loadImg('+i+')" class="nft':
				myWH[i][7]+'" class="nobreed')
			 +'"></p>'
		);
	}
}
...
async function getCID(){
	...
	var img;
	await $.getJSON('https://raw.githubusercontent.com/aloycwl/WhooliHootie/main/img.json',function(d){img=d.g[gen].s[sex]});
