...
contract ERC721AC is IERC721,IERC721Metadata{
    ...
    function Mint(address _a, uint256 _generation)internal{
        unchecked{
            ...
            ac[count].generation=_generation;
    ...
    function breed(uint256 p1,uint256 p2)external{
        bool existed; //never mint before
        for(uint256 i=0;i<tokens[msg.sender].length;i++){
            if((ac[tokens[msg.sender][i]].parent1==p1&&ac[tokens[msg.sender][i]].parent2==p2)||
            (ac[tokens[msg.sender][i]].parent2==p1&&ac[tokens[msg.sender][i]].parent1==p2))existed==true;
        }
        require(!paused&&
        breedable&&
        ac[p1].generation==ac[p2].generation&& //must be same generation
        age[ac[p1].generation].currentCount<=age[ac[p1].generation].maxCount&& //same generation count not exhausted
        ownerOf(p1)==msg.sender&&ownerOf(p2)==msg.sender&& //must only owner of p1 and p2
        ac[p1].time+WAITINGTIME<block.timestamp&&ac[p2].time+WAITINGTIME<block.timestamp&& //parents haven't been minted recently
        (ac[p1].gender==1&&ac[p2].gender==2)||(ac[p2].gender==1&&ac[p1].gender==2)&& //must be different gender
        !existed);
        Mint(msg.sender,2);
        ac[count].parent1=p1;
        ac[count].parent2=p2;
